    ...
    switch (tap_state.state) {
        case TD_SINGLE_TAP:
            if (user_config.os == MACOS) {
                dprintln("Executing MACOS copy command");
                tap_code16(G(KC_C));
            } else {
                dprintln("Executing WINDOWS copy command");
                tap_code16(C(KC_C));
            }
            break;
        case TD_SINGLE_HOLD:
            if (user_config.os == MACOS) {
                dprintln("Executing MACOS copy hold command");
                tap_code16(G(KC_X));
            } else {
                dprintln("Executing WINDOWS copy hold command");
                tap_code16(C(KC_X));
            }
            break;
        default: break;
